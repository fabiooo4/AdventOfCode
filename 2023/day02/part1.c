...
int main() {
  ...
  for (int lineIdx = 1; fgets(line, MAX, input); lineIdx++) {
    ...
    if (game[lineIdx].maxRed <= MAX_RED_VALUE &&
        game[lineIdx].maxGreen <= MAX_GREEN_VALUE &&
        game[lineIdx].maxBlue <= MAX_BLUE_VALUE) {
      finalAnswer += game[lineIdx].id;
    }
  }
